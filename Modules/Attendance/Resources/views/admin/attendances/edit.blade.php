...
@section('content-header')
    <h1>
        {{ trans('attendance::attendances.title.edit attendance') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.attendance.attendance.index') }}">{{ trans('attendance::attendances.title.attendances') }}</a></li>
        <li class="active">{{ trans('attendance::attendances.title.edit attendance') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.attendance.attendance.update', $attendance->id], 'method' => 'put']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('attendance::admin.attendances.partials.edit-fields', ['lang' => $locale])
                        </div>
                    @endforeach
